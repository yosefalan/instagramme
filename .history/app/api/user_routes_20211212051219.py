...
# GET USERS FOR FOLLOWING FEED

@user_routes.route('/fallowing-feed')
def get_following():
    userId = session['_user_id']
    user = User.query.get(userId).to_dict()
    results = User.query.filter(User.user_id.in_(
        user["following"])).all()

    results_dict = {user.id: post.to_dict() for post in results}
    return results_dict
